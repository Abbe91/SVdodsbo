...
const Flyttstadning = ({ videoText }) => {
  const [modalOpen, setModalOpen] = useState(false);
  const mainquery = "flyttstädning";
  ...
  const [content, setContent] = useState({
    ...
    subTitle2:
      "Din auktoriserade hanterare av flyttstädning, dödsbo, uppköp av lösöre, försäljning av bohag, tömning av bohag, bortforsling av dödsbo och bohag, sanering och flytt",
  ...
  return (
    <div>
      ...
      <Helmet>
        ...
        <link rel="canonical" href="https://alladodsbo.se/flyttstadning" />
      </Helmet>
      <div className="Nav">
        <Nav />
      </div>
      ...
      <div className="Om">
        <AboutUs
          ...
          om={`är det självklara valet för hantering all typ av städning så som flyttstädning, städning av dödsbo och bohag.`}
        />
      </div>
      {newsData && (
        <div className="Tips">
