...
const Stadardodsboalingsas = ({ videoText }) => {
  const [modalOpen, setModalOpen] = useState(false);
  const mainquery = "Städar dödsbo Alingsås";
  const newsData = useSelector(getNewsData);
  const bingNews = useSelector(getBingNews);
  const [content, setContent] = useState({
    metaTitle: "Städar dödsbo Alingsås. Billigt och smidigt.",
    metaContent:
      "Städar dödsbo Alingsås. Din auktoriserade städfirma som städar dödsbo i ALingsås.",
  ...
  const services =
    "uppköp, tömning, bortforsling, sanering, luktsanering, flytt, försäljning, och städning av dödsbo  i Alingsås";
...
export default Stadardodsboalingsas;
