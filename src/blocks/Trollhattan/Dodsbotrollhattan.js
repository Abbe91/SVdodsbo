...
const Dodsbotrollhattan = ({ videoText }) => {
  const [modalOpen, setModalOpen] = useState(false);
  const mainquery = "Dödsbo Trollhättan";
  const newsData = useSelector(getNewsData);
  const bingNews = useSelector(getBingNews);
  const [content, setContent] = useState({
    metaTitle: "Dödsbo Trollhättan. Pålitlig värdering & tömning",
    metaContent:
      "Dödsbo Trollhättan. Behöver du hjälp med att hanetra ett dödsbo i Trollhättan? Kontakta oss idag. ",
    query: "Dödsbo Trollhättan",
    formTitle: "Dödsbo Trollhättan",
    subTitle2:
      "Din auktoriserade hanterare av dödsbo, bohag, uppköp, försäljning, tömning, bortforsling, sanering, flytt och städning av dödsbo i Trollhättan",
  ...
  return (
    <div>
      <Helmet>
        ...
        <link rel="canonical" href="https://alladodsbo.se/dodsbo-trollhattan" />
      </Helmet>
      <div className="Nav">
        <Nav />
      </div>
...
export default Dodsbotrollhattan;
