...
const Saljadodsbotrollhattan = ({ videoText }) => {
  const [modalOpen, setModalOpen] = useState(false);
  const mainquery = "Sälja dödsbo Trollhättan";
  ...
  const [content, setContent] = useState({
    ...
    subTitle2:
      "Din auktoriserade hanterare av dödsbo, bohag, uppköp, tömning, bortforsling, sanering, flytt, städning och försäljning av dödsbo i Trollhättan",
    subTitle3: `Behöver du professionell hjälp med ${mainquery}`,
  });
  const customerData = useSelector(getCustomerData);

  const services =
    "uppköp, tömning, bortforsling, sanering, luktsanering, flytt, städ, försäljning av dödsbo i Trollhättan";
  ...
  return (
    <div>
      <Helmet>
        ...
        <link
          rel="canonical"
          href="https://alladodsbo.se/salja-dodsbo-trollhattan"
        />
      </Helmet>
      <div className="Nav">
        <Nav />
...
export default Saljadodsbotrollhattan;
