...
const Stadadodsbogoteborg = ({ videoText }) => {
  const [modalOpen, setModalOpen] = useState(false);
  const mainquery = "Städa dödsbo Göteborg";
  const newsData = useSelector(getNewsData);
  const bingNews = useSelector(getBingNews);
  const [content, setContent] = useState({
    metaTitle:
      "Städa dödsbo Göteborg. Effektivt och pålitligt städning av dödsbo i Göteborg",
    metaContent:
      "Vi erbjuder professionella, effektiv och pålitlig städning av dödsbo i Göteborg. Kontakta oss idag för att boka en städning av ett dödsbo i Göteborg.",
    ...
    <div>
      ...
      <Helmet>
        ...
        <link
          hrefLang="sv"
          rel="canonical"
          href="https://alladodsbo.se/stada-dodsbo-goteborg"
        />
      </Helmet>
      <div className="Nav">
        <Nav />
...
export default Stadadodsbogoteborg;
