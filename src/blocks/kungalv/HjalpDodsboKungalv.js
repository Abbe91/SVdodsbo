...
const HjalpDodsboKungalv = ({ videoText }) => {
  const [modalOpen, setModalOpen] = useState(false);
  const mainquery = "hjälp dödsbo Kungälv";
  const newsData = useSelector(getNewsData);
  const bingNews = useSelector(getBingNews);
  const [content, setContent] = useState({
    metaTitle: "Hjälp dödsbo Kungälv",
    metaContent: "Hjälp dödsbo Kungälv",
    query: "Hjälp dödsbo Kungälv",
    formTitle: "Hjälp dödsbo Kungälv",
    subTitle2:
    ...
    <div>
      <Helmet>
        ...
        <link
          hrefLang="sv"
          rel="canonical"
          href="https://alladodsbo.se/hjalp-dodsbo-kungalv"
        />
      </Helmet>
      <div className="Nav">
        <Nav />
...
export default HjalpDodsboKungalv;
