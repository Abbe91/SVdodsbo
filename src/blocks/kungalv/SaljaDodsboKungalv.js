...
const SaljaDodsboKungalv = ({ videoText }) => {
  const [modalOpen, setModalOpen] = useState(false);
  const mainquery = "sälja dödsbo Kungälv";
  ...
  const [content, setContent] = useState({
    ...
    subTitle2:
      "Din auktoriserade hanterare av dödsbo, bohag, uppköp, tömning, bortforsling, sanering, flytt, städning och försäljning av dödsbo i Kungälv",
    subTitle3: `Behöver du professionell hjälp med ${mainquery}`,
  });
  const customerData = useSelector(getCustomerData);

  const services =
    "uppköp, tömning, bortforsling, sanering, luktsanering, flytt, städning, försäljning ch försäljning av dödsbo i Kungälv";
    ...
    <div>
      <Helmet>
        ...
        <link
          hrefLang="sv"
          rel="canonical"
          href="https://alladodsbo.se/salja-dodsbo-kungalv"
        />
        <meta
          property="og:url"
          content="https://alladodsbo.se/salja-dodsbo-kungalv"
        />
      </Helmet>
      <div className="Nav">
        <Nav />
...
export default SaljaDodsboKungalv;
