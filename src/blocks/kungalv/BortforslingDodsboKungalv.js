...
const BortforslingDodsboKungalv = ({ videoText }) => {
  const [modalOpen, setModalOpen] = useState(false);
  const mainquery = "bortforsling dödsbo Kungälv";
  const newsData = useSelector(getNewsData);
  const bingNews = useSelector(getBingNews);
  const [content, setContent] = useState({
    metaTitle:
      "Effektiv bortforsling av dödsbo i Kungälv - smidig och stressfri process",
    metaContent:
      "Behöver du hjälp med bortforsling av ett dödsbo i Kungälv? Kontakta oss idag för en smidig och stressfri lösning för bortforsling av dödsbo i Kungälv.",
  ...
  return (
    <div>
      <Helmet>
        ...
        <link
          rel="canonical"
          href="https://alladodsbo.se/bortforsling-dodsbo-kungalv"
        />
      </Helmet>
      <div className="Nav">
        <Nav />
...
export default BortforslingDodsboKungalv;
