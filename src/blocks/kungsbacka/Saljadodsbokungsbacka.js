...
const Saljadodsbokungsbacka = ({ videoText }) => {
  const [modalOpen, setModalOpen] = useState(false);
  const mainquery = "Sälja dödsbo Kungsbacka";
  ...
  const [content, setContent] = useState({
    ...
    subTitle2:
      "Din auktoriserade hanterare av dödsbo, bohag, uppköp, försäljning, tömning, bortforsling, sanering, flytt, städning och sälja dödsbo i Kungsbacka",
    subTitle3: `Behöver du professionell hjälp med ${mainquery}`,
  });
  const customerData = useSelector(getCustomerData);

  const services =
    "uppköp, tömning, bortforsling, sanering, luktsanering, flytt, städ, försäljning och sälja dödsbo i Kungsbacka";
  ...
  return (
    <div>
      <Helmet>
        ...
        <link
          rel="canonical"
          href="https://alladodsbo.se/salja-dodsbo-kungsbacka"
        />
      </Helmet>
      <div className="Nav">
        <Nav />
...
export default Saljadodsbokungsbacka;
