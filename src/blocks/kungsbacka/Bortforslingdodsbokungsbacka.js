...
const Bortforslingdodsbokungsbacka = ({ videoText }) => {
  const [modalOpen, setModalOpen] = useState(false);
  const mainquery = "Bortforsling dödsbo Kungsbacka";
  const newsData = useSelector(getNewsData);
  const bingNews = useSelector(getBingNews);
  const [content, setContent] = useState({
    metaTitle:
      "Effektiv bortforsling av dödsbo i Kungsbacka - smidig och stressfri process",
    metaContent:
      "Behöver du hjälp med bortforsling av ett dödsbo i Kungsbacka? Kontakta oss idag för en smidig och stressfri lösning för bortforsling av dödsbo i Kungsbacka.",
  ...
  const services =
    "uppköp, tömning, bortforsling, sanering, luktsanering, flytt, städ, försäljning och bortforsling av dödsbo i Kungsbacka";
...
export default Bortforslingdodsbokungsbacka;
