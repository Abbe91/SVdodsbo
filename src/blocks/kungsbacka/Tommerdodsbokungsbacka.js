...
const Tommerdodsbokungsbacka = ({ videoText }) => {
  const [modalOpen, setModalOpen] = useState(false);
  const mainquery = "Tömmer dödsbo Kungsbacka";
  ...
  const [content, setContent] = useState({
    ...
    subTitle2:
      "Din auktoriserade hanterare av dödsbo, bohag, uppköp, försäljning, tömning, bortforsling, sanering, flytt, städning och tömning av dödsbo i Kungsbacka",
    subTitle3: `Behöver du professionell hjälp med ${mainquery}`,
  });
  const customerData = useSelector(getCustomerData);

  const services =
    "uppköp, tömning, bortforsling, sanering, luktsanering, flytt, städning städning och tömning av dödsbo i Kungsbacka";
  ...
  return (
    <div>
      ...
      <Helmet>
        ...
        <meta
          property="og:url"
          content="https://alladodsbo.se/tommer-dodsbo-kungsbacka"
        />
        <link
          hrefLang="sv"
          rel="canonical"
          href="https://alladodsbo.se/tommer-dodsbo-kungsbacka"
        />
        <meta
          property="og:image"
...
export default Tommerdodsbokungsbacka;
