...
const TommadodsboKungsbacka = ({ videoText }) => {
  const [modalOpen, setModalOpen] = useState(false);
  const mainquery = "Tömma dödsbo Kungsbacka";
  const newsData = useSelector(getNewsData);
  const bingNews = useSelector(getBingNews);
  const [content, setContent] = useState({
    metaTitle:
      "Tömma dödsbo i Kungsbacka - Professionell service för en stressfri process!",
    metaContent:
    ...
    subTitle2:
      "Din auktoriserade hanterare av dödsbo, bohag, uppköp, försäljning, bortforsling, sanering, flytt, städning och tömning av dödsbo i Kungsbacka",
    subTitle3: `Behöver du professionell hjälp med ${mainquery}`,
  });
  const customerData = useSelector(getCustomerData);

  const services =
    "uppköp, tömning, bortforsling, sanering, luktsanering, flytt, städning och tömning av dödsbo i Kungsbacka";
  ...
  return (
    <div>
      ...
      <Helmet>
        ...
        <meta
          property="og:url"
          content="https://alladodsbo.se/tomma-dodsbo-kungsbacka"
        />
        <link
          rel="canonical"
          href="https://alladodsbo.se/tomma-dodsbo-kungsbacka"
        />
      </Helmet>
      <div className="Nav">
        <Nav />
...
export default TommadodsboKungsbacka;
