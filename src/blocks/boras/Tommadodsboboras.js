...
const Tommadodsboboras = ({ videoText }) => {
  const [modalOpen, setModalOpen] = useState(false);
  const mainquery = "Tömma dödsbo Borås";
  ...
  const [content, setContent] = useState({
    ...
    subTitle2:
      "Din auktoriserade hanterare av dödsbo, bohag, uppköp, försäljning, bortforsling, sanering, flytt, städning och tömning av dödsbo  i Borås",
    subTitle3: `Behöver du professionell hjälp med ${mainquery}`,
  });
  const customerData = useSelector(getCustomerData);

  const services =
    "uppköp, bortforsling, sanering, luktsanering, flytt, städ, försäljning och tömning av dödsbo i Borås";
  ...
  return (
    <div>
      ...
      <Helmet>
        ...
        <link rel="canonical" href="https://alladodsbo.se/tomma-dodsbo-boras" />
      </Helmet>
      <div className="Nav">
        <Nav />
      </div>
...
export default Tommadodsboboras;
