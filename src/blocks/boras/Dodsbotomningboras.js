...
const Dodsbotomningboras = ({ videoText }) => {
  const [modalOpen, setModalOpen] = useState(false);
  const mainquery = "Dödsbo tömning Borås";
  const newsData = useSelector(getNewsData);
  const bingNews = useSelector(getBingNews);
  const [content, setContent] = useState({
    metaTitle: "Dödsbo tömning Borås. Prisvärt, Snabbt och Pålitligt.",
    metaContent:
      "Dödsbo tömning Borås. Din auktoriserade firma som utför tömningar av dödsbon i Borås",
    query: "Dödsbo tömning Borås",
    formTitle: "Dödsbo tömning Borås",
    subTitle2:
      "Din auktoriserade hanterare av dödsbo, bohag, uppköp, försäljning, tömning, bortforsling, sanering, flytt, städning och tömning av dödsbo i Borås",
    subTitle3: `Behöver du professionell hjälp med ${mainquery}`,
  });
  const customerData = useSelector(getCustomerData);

  const services =
    "uppköp, bortforsling, sanering, flytt, städning, försäljning och tömning av dödsbo och bohag i Borås";
    ...
    <div>
      <Helmet>
        ...
        <link
          hrefLang="sv"
          rel="canonical"
          href="https://alladodsbo.se/dodsbo-tomning-boras"
        />
      </Helmet>
      <div className="Nav">
        <Nav />
...
export default Dodsbotomningboras;
