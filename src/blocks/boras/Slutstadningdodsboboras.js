...
const Slutstadningdodsboboras = ({ videoText }) => {
  const [modalOpen, setModalOpen] = useState(false);
  const mainquery = "Slutstädning dödsbo Borås";
  const newsData = useSelector(getNewsData);
  const bingNews = useSelector(getBingNews);
  const [content, setContent] = useState({
    metaTitle:
      "Slutstädning dödsbo Borås. Din auktoriserade hanterare av dödsbo, bohag, uppköp, försäljning, tömning, bortforsling, sanering, flytt och slutstädning av dödsbo i Borås",
    metaContent:
      "Slutstädning dödsbo Borås. Din auktoriserade hanterare av dödsbo, bohag, uppköp, försäljning, tömning, bortforsling, sanering, flytt och slutstädning av dödsbo i Borås",
    query: "Slutstädning dödsbo Borås",
    formTitle: "Slutstädning dödsbo Borås",
    subTitle2:
      "Din auktoriserade hanterare av dödsbo, bohag, uppköp, försäljning, tömning, bortforsling, sanering, flytt och slutstädning av dödsbo i Borås",
    subTitle3: `Behöver du professionell hjälp med ${mainquery}`,
  });
  const customerData = useSelector(getCustomerData);

  const services =
    "uppköp, tömning, bortforsling, sanering, luktsanering, flytt, städ, försäljning och slutstädning av dödsbo i Borås";
  ...
  return (
    <div>
      <Helmet>
        ...
        <link
          rel="canonical"
          href="https://alladodsbo.se/slutstadning-dodsbo-boras"
        />
      </Helmet>
      <div className="Nav">
        <Nav />
