...
const Bortforslingdodsboboras = ({ videoText }) => {
  const [modalOpen, setModalOpen] = useState(false);
  const mainquery = "bortforsling dödsbo Borås";
  ...
  const [content, setContent] = useState({
    ...
    subTitle2:
      "Din auktoriserade hanterare av dödsbo, bohag, uppköp, försäljning, tömning, sanering, flytt, städning och bortforsling av dödsbo i Borås",
    subTitle3: `Behöver du professionell hjälp med ${mainquery}`,
  });
  const customerData = useSelector(getCustomerData);

  const services =
    "uppköp, tömning, sanering, luktsanering, flytt, städning, försäljning och bortforsling av dödsbo i Borås";
    ...
    <div>
      <Helmet>
        ...
        <link
          hrefLang="sv"
          rel="canonical"
          href="https://alladodsbo.se/bortforsling-dodsbo-boras"
        />
      </Helmet>
      <div className="Nav">
        <Nav />
...
export default Bortforslingdodsboboras;
