...
const Rojadodsboboras = ({ videoText }) => {
  const [modalOpen, setModalOpen] = useState(false);
  const mainquery = "Röja dödsbo Borås";
  const newsData = useSelector(getNewsData);
  const bingNews = useSelector(getBingNews);
  const [content, setContent] = useState({
    metaTitle: "Röja dödsbo Borås. Snabbt & Prisvärt!",
    metaContent:
      "Röja dödsbo Borås. Vi röjer dödsbo i Borås och ger dig en snabb och effektiv service.",
    query: "Röja dödsbo Borås",
    formTitle: "Röja dödsbo Borås",
    subTitle2:
      "Din auktoriserade hanterare av dödsbo, bohag, uppköp, försäljning, tömning, bortforsling, sanering, flytt, städning och röjning av dödsbo i Borås",
    subTitle3: `Behöver du professionell hjälp med ${mainquery}`,
  });
  const customerData = useSelector(getCustomerData);

  const services =
    "uppköp, tömning, bortforsling, sanering, luktsanering, flytt, städ, försäljning och röjning av dödsbo i Borås";
  ...
  return (
    <div>
      <Helmet>
        ...
        <link rel="canonical" href="https://alladodsbo.se/roja-dodsbo-boras" />
      </Helmet>
      <div className="Nav">
        <Nav />
      </div>
...
export default Rojadodsboboras;
