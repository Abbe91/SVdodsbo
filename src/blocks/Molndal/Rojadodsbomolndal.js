...
const Rojadodsbomolndal = ({ videoText }) => {
  const [modalOpen, setModalOpen] = useState(false);
  const mainquery = "Röja dödsbo Mölndal";
  const newsData = useSelector(getNewsData);
  const bingNews = useSelector(getBingNews);
  const [content, setContent] = useState({
    metaTitle: "Röja dödsbo Mölndal. Snabbt & Prisvärt!",
    metaContent:
      "Röja dödsbo Mölndal. Vi röjer dödsbo i Mölndal och ger dig en snabb och effektiv service.",
  ...
  return (
    <div>
      <Helmet>
        ...
        <link
          rel="canonical"
          href="https://alladodsbo.se/roja-dodsbo-molndal"
        />
      </Helmet>
      <div className="Nav">
        <Nav />
...
export default Rojadodsbomolndal;
