...
const Stadadodsbomolndal = ({ videoText }) => {
  const [modalOpen, setModalOpen] = useState(false);
  const mainquery = "Städa dödsbo Mölndal";
  const newsData = useSelector(getNewsData);
  const bingNews = useSelector(getBingNews);
  const [content, setContent] = useState({
    metaTitle: "Städa dödsbo Mölndal. Rent & Prisvärt!",
    metaContent:
      "Städa dödsbo Mölndal. Behöver du hjälp med att städa ett dödsbo i Mölndal? Ring oss idag!",
  ...
  return (
    <div>
      <Helmet>
        ...
        <link
          rel="canonical"
          href="https://alladodsbo.se/stada-dodsbo-molndal"
        />
      </Helmet>
      <div className="Nav">
        <Nav />
...
export default Stadadodsbomolndal;
