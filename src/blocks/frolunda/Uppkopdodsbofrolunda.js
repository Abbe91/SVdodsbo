...
const Uppkopdodsbofrolunda = ({ videoText }) => {
  const [modalOpen, setModalOpen] = useState(false);
  const mainquery = "uppköp dödsbo Västra Frölunda";
  ...
  const [content, setContent] = useState({
    ...
    subTitle2:
      "Din auktoriserade hanterare av dödsbo, bohag, försäljning, tömning, bortforsling, sanering, flytt, städning och uppköp av dödsbo i Frölunda",
    subTitle3: `Behöver du professionell hjälp med ${mainquery}`,
  });
  const customerData = useSelector(getCustomerData);

  const services =
    "värdering, tömning, bortforsling, sanering, luktsanering, flytt, städ, försäljning och uppköp av dödsbo i Frölunda";
  ...
  return (
    <div>
      <Helmet>
        ...
        <link
          rel="canonical"
          href="https://alladodsbo.se/uppkop-dodsbo-frolunda"
        />
      </Helmet>
      <div className="Nav">
        <Nav />
...
export default Uppkopdodsbofrolunda;
