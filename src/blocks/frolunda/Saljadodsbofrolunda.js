...
const Saljadodsbofrolunda = ({ videoText }) => {
  const [modalOpen, setModalOpen] = useState(false);
  const mainquery = "sälja dödsbo Frölunda";
  ...
  const [content, setContent] = useState({
    ...
    subTitle2:
      "Din auktoriserade hanterare av dödsbo, bohag, uppköp, tömning, bortforsling, sanering, flytt, städning och försäljning av dödsbo i Frölunda",
    subTitle3: `Behöver du professionell hjälp med ${mainquery}`,
  });
  const customerData = useSelector(getCustomerData);

  const services =
    "uppköp, tömning, bortforsling, sanering, flytt, städ, värdering och försäljning av dödsbo i Frölunda";
  ...
  return (
    <div>
      <Helmet>
        ...
        <link
          rel="canonical"
          href="https://alladodsbo.se/salja-dodsbo-frolunda"
        />
      </Helmet>
      <div className="Nav">
        <Nav />
...
export default Saljadodsbofrolunda;
