...
const Varderadodsbofrolunda = ({ videoText }) => {
  const [modalOpen, setModalOpen] = useState(false);
  const mainquery = "värdera dödsbo Frölunda";
  ...
  const [content, setContent] = useState({
    ...
    subTitle2:
      "Din auktoriserade hanterare av dödsbo, bohag, uppköp, försäljning, tömning, bortforsling, sanering, flytt, städning och värdering av dödsbo i Frölunda",
    subTitle3: `Behöver du professionell hjälp med ${mainquery}`,
  });
  const customerData = useSelector(getCustomerData);

  const services =
    "uppköp, tömning, bortforsling, sanering, luktsanering, flytt, städ, försäljning och värdering av dödsbo i Frölunda";
  ...
  return (
    <div>
      <Helmet>
        ...
        <link
          rel="canonical"
          href="https://alladodsbo.se/vardera-dodsbo-frolunda"
        />
      </Helmet>
      <div className="Nav">
        <Nav />
...
export default Varderadodsbofrolunda;
