...
const Stadadodsbopartille = ({ videoText }) => {
  const [modalOpen, setModalOpen] = useState(false);
  const mainquery = "städa dödsbo Partille";
  const newsData = useSelector(getNewsData);
  const bingNews = useSelector(getBingNews);
  const [content, setContent] = useState({
    metaTitle: "Städa dödsbo Partille. Rent & Prisvärt!",
    metaContent:
      "Städa dödsbo Partille. Behöver du hjälp med att städa ett dödsbo i Partille? Ring oss idag!",
    query: "Städa dödsbo Partille",
    formTitle: "Städa dödsbo Partille",
    subTitle2:
      "Din auktoriserade hanterare av dödsbo, bohag, uppköp, försäljning, tömning, bortforsling, sanering, flytt och städning av dödsbo i Partille",
    subTitle3: `Behöver du professionell hjälp med ${mainquery}`,
  });
  const customerData = useSelector(getCustomerData);

  const services =
    "uppköp, tömning, bortforsling, sanering, luktsanering, flytt, försäljning och städning av dödsbo i Partille";
  ...
  return (
    <div>
      <Helmet>
        ...
        <link
          rel="canonical"
          href="https://alladodsbo.se/stada-dodsbo-partille"
        />
      </Helmet>
      <div className="Nav">
        <Nav />
...
export default Stadadodsbopartille;
