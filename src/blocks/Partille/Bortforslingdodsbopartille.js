...
const Bortforslingdodsbopartille = ({ videoText }) => {
  const [modalOpen, setModalOpen] = useState(false);
  const mainquery = "bortforsling dödsbo Partille";
  const newsData = useSelector(getNewsData);
  const bingNews = useSelector(getBingNews);
  const [content, setContent] = useState({
    metaTitle:
      "Effektiv bortforsling av dödsbo i Partille - smidig och stressfri process",
    metaContent:
      "Behöver du hjälp med bortforsling av ett dödsbo i Partille? Kontakta oss idag för en smidig och stressfri lösning för bortforsling av dödsbo i Partille.",
    query: "Bortforsling dödsbo Partille",
    query: "Bortforsling dödsbo Partille",
    formTitle: "Bortforsling dödsbo Partille",
    subTitle2:
      "Din auktoriserade hanterare av dödsbo, bohag, uppköp, försäljning, tömning, sanering, flytt, städ och bortforsling av dödsbo i Partille",
    subTitle3: `Behöver du professionell hjälp med ${mainquery}`,
  });
  const customerData = useSelector(getCustomerData);

  const services =
    "uppköp, tömning, sanering, luktsanering, flytt, städ, försäljning och bortforsling av dödsbo i Partille";
  ...
  return (
    <div>
      <Helmet>
        ...
        <link
          rel="canonical"
          href="https://alladodsbo.se/bortforsling-dodsbo-partille"
        />
      </Helmet>
      <div className="Nav">
        <Nav />
...
export default Bortforslingdodsbopartille;
