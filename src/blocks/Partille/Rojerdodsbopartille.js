...
const Rojerdodsbopartille = ({ videoText }) => {
  const [modalOpen, setModalOpen] = useState(false);
  const mainquery = "röjer dödsbo Partille";
  const newsData = useSelector(getNewsData);
  const bingNews = useSelector(getBingNews);
  const [content, setContent] = useState({
    metaTitle: "Röja dödsbo Partille. Snabbt & Prisvärt!",
    metaContent:
      "Röja dödsbo Partille. Vi röjer dödsbo i Partille och ger dig en snabb och effektiv service.",
  ...
  const services =
    "uppköp, tömning, bortforsling, sanering, luktsanering, flytt, städ, försäljning, röjning av dödsbo i Partille";
  ...
  return (
    <div>
      <Helmet>
        ...
        <link
          rel="canonical"
          href="https://alladodsbo.se/rojer-dodsbo-partille"
        />
      </Helmet>
      <div className="Nav">
        <Nav />
...
export default Rojerdodsbopartille;
