...
const Tommadodsboale = ({ videoText }) => {
  const [modalOpen, setModalOpen] = useState(false);
  const mainquery = "tömma dödsbo Ale";
  const newsData = useSelector(getNewsData);
  const bingNews = useSelector(getBingNews);
  const [content, setContent] = useState({
    metaTitle:
      "Säker och smidig tömning av dödsbo i Ale - Professionell service för en stressfri process",
    metaContent:
      "Behöver du hjälp med att tömma ett dödsbo i Ale? Vår erfarna personal garanterar en prisvärd, smidig och säker process.",
    query: "Tömma dödsbo Ale",
    formTitle: "Tömma dödsbo Ale",
    subTitle2:
      "Din auktoriserade hanterare av dödsbo, bohag, uppköp, försäljning, bortforsling, sanering, flytt, städning och tömning av dödsbo i Ale",
    subTitle3: `Behöver du professionell hjälp med ${mainquery}`,
  });
  const customerData = useSelector(getCustomerData);

  const services =
    "uppköp, tömning, bortforsling, sanering, luktsanering, flytt, städ, försäljning och tömning av dödsbo i Ale";
  const articleStructuredData = {
    ...
    datePublished: new Date("2023-04-16T09:25:01.340Z").toISOString(),
    author: {
      "@type": "Person",
      name: "DigitalTech",
    },
    ...
    <div>
      ...
      <Helmet>
        ...
        <link
          hrefLang="sv"
          rel="canonical"
          href="https://alladodsbo.se/tomma-dodsbo-ale"
        />
      </Helmet>
      <div className="Nav">
        <Nav />
