...
const Uppkopdodsboale = ({ videoText }) => {
  const [modalOpen, setModalOpen] = useState(false);
  const mainquery = "uppköp dödsbo Ale";
  ...
  const [content, setContent] = useState({
    ...
    subTitle2:
      "Din auktoriserade hanterare av dödsbo, bohag, uppköp, försäljning, tömning, bortforsling, sanering, flytt, städning och uppköp av dödsbo i Ale",
    subTitle3: `Behöver du professionell hjälp med ${mainquery}`,
  });
  const customerData = useSelector(getCustomerData);

  const services =
    "uppköp, tömning, bortforsling, sanering, luktsanering, flytt, städ, försäljning och uppköp av dödsbo i Ale";
    ...
    <div>
      ...
      <Helmet>
        ...
        <link
          hrefLang="sv"
          rel="canonical"
          href="https://alladodsbo.se/uppkop-dodsbo-ale"
        />
      </Helmet>
      <div className="Nav">
        <Nav />
