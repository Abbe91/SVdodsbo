...
const Uppkopdodsbohisingen = ({ videoText }) => {
  const [modalOpen, setModalOpen] = useState(false);
  const mainquery = "uppköp dödsbo Hisingen";
  const newsData = useSelector(getNewsData);
  const bingNews = useSelector(getBingNews);
  const [content, setContent] = useState({
    metaTitle:
      "Uppköp av dödsbo i Hisingen. Vill du sälj ett dödsbo i Hisingen? Vi köper hela eller delar av dödsbon till rättvist pris.",
    query: "Uppköp dödsbo Hisingen",
    metaContent:
      "Uppköp av dödsbo i Hisingen. Sälj dödsbo i Hisingen till rättvist pris med vår professionella uppköpstjänst. Vi köper hela eller delar av dödsbon i Hisingen.",
    query: "Uppköp dödsbo Hisingen",
    formTitle: "Uppköp dödsbo Hisingen",
    subTitle2:
  ...
  return (
    <div>
      ...
      <Helmet>
        ...
        <link
          rel="canonical"
          href="https://alladodsbo.se/uppkop-dodsbo-hisingen"
        />
      </Helmet>
      <div className="Nav">
        <Nav />
...
export default Uppkopdodsbohisingen;
