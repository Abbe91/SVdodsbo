...
const Bortforslinghisingen = ({ videoText }) => {
  const [modalOpen, setModalOpen] = useState(false);
  const mainquery = "bortforsling dödsbo Hisingen";
  const newsData = useSelector(getNewsData);
  const bingNews = useSelector(getBingNews);
  const [content, setContent] = useState({
    metaTitle: "Bortforsling av dödsbo i Hisingen - Effektivt & prisvärt",
    metaContent:
      "Effektiv bortforsling av dödsbo i Hisingen -  Låt oss sköta bortforslingen av ert dödsbo i Hisingen.",
...
export default Bortforslinghisingen;
